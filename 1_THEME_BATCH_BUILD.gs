function sortThemeAndBatchFull() {
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('OpenEndCoding');
  const sheet2 = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Themes');
  ...
  // Get the Prompt content from cell L2
  const userContent = sheet.getRange('L2').getValue();

  const GPT_API = "ADD GPT API KEY";
  ...
  // Log the result and place it into cell I2
  console.log(result);
  ...
  if (fileId) {
    ...
    const TIMEOUT_LIMIT = 1.2 * 60 * 1000; // 1.2 minutes in milliseconds (DEFAULT SHOULD BE 1.2 * 60...)
  ...
  Logger.log('sortThemeAndBatch completed.');
  // Call the second function at the end of the first function
  fullUseBatchFileToBuildQCodeFrame();
}

function fullUseBatchFileToBuildQCodeFrame() {
 ...
 /////// NOW WE WILL BUILD THE Q CODE FRAME
  ...
  // Read themes from cell K4 "Since this includes the "Nonsense" theme
...
function startProcessFull() {
  sortThemeAndBatchFull();
}
